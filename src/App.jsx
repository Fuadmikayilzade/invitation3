...
/* ── CURTAIN: fixed overlay — fine, it's not a scroll container ── */
.curtain{
...
.cv-hint{
  position:absolute; left:0; right:0; bottom:14svh; z-index:3;
...
/* ── INVITE: normal scroll container ── */
.invite{
  position:relative;
  z-index:1;
  min-height:100%;
...
.invite.hidden{ opacity:0; pointer-events:none; }

/* floral bg — fixed so it doesn't scroll with content */
.floral-bg{
  position:fixed; inset:0;
...
.hero-overlay{
  ...
  background:linear-gradient(180deg,
    rgba(247,245,242,.6) 0%,rgba(247,245,242,.1) 35%,
    rgba(247,245,242,.1) 65%,rgba(247,245,242,.88) 100%);
}
.hero-body{
  position:relative; z-index:2;
  padding:28px 40px;
...
.hero-names{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(52px,13vw,96px); font-weight:300;
  line-height:.88; color:#0e1e5a;
}
.hero-names em{ display:block; font-style:italic; color:#1e3470; }
.hero-amp{
  display:block; font-family:'Cormorant Garamond',serif;
  font-size:.44em; font-style:italic;
  color:rgba(58,90,155,.35); margin:.16em 0;
}
.hero-date{
...
.hero-scroll{
  ...
  font-size:clamp(13px,3vw,14px); color:rgba(58,90,155,.45);
  animation:breathe 3s ease-in-out infinite;
}
...
.cd-num{
  ...
  font-size:clamp(28px,7vw,44px); font-weight:500;
  color:#102060; line-height:1; margin-bottom:8px;
}
.cd-lbl{
  font-family:'Cormorant Garamond',serif;
...
.scratch-reveal{
  ...
  object-fit:contain; object-position:center; background:#dce8f5; display:block;
}
.scratch-canvas{ position:absolute; inset:0; width:100%; height:100%; touch-action:none; }
.scratch-hint{
  font-family:'Cormorant Garamond',serif; font-style:italic;
  font-size:clamp(13px,3.2vw,15px); color:var(--muted); margin-top:18px;
}
.scratch-msg{
  font-family:'Cormorant Upright',serif;
...
.dress-title{
  ...
  color:var(--blue-d); letter-spacing:.06em; margin-bottom:10px;
}
.dress-divider{
  width:36px; height:1px;
  background:linear-gradient(90deg,transparent,var(--line),transparent);
...
/* ── RSVP ── */
.rsvp-sec{
  padding:0; position:relative; overflow:hidden; text-align:center;
}
.rsvp-vid-bg{
  position:absolute; inset:0;
  width:100%; height:100%; object-fit:cover; opacity:1; z-index:0;
}
.rsvp-vid-overlay{
  position:absolute; inset:0;
  background:rgba(247,245,242,.45); z-index:1;
}
...
.f-choice.sel{ border-color:rgba(58,90,155,.6); background:rgba(58,90,155,.08); color:var(--blue-d); }
.guests-list{ display:flex; flex-direction:column; gap:8px; }
.g-row{ display:flex; gap:8px; align-items:center; }
.g-row .f-input{ flex:1; }
.g-btn{
...
.footer-name{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(16px,4vw,20px); font-weight:300;
  font-style:italic; color:rgba(58,90,155,.65);
}
...
#conf-canvas{ position:fixed; inset:0; pointer-events:none; z-index:9999; }
`;
...
export default function App(){
  ...
  return (
    <>
      ...
      {}
      <div
        className={`curtain${screen==='invite'?' hidden':''}`}
        onClick={openCurtain}
      >
        <video ref={cvRef} className="cv-vid"
          playsInline webkit-playsinline="true" muted preload="auto">
      ...
      {}
      <div
        className={`invite${screen==='invite'?'':' hidden'}`}
        style={{ '--floral':`url(${IMG_FLORAL})`, '--table':`url(${IMG_TABLE})` }}
      >
        {/* floral background */}
        <div className="floral-bg"/>
