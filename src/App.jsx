...
.cv-hint{
  position:absolute; left:0; right:0; bottom:14svh;
  z-index:3;
  display:flex; flex-direction:column;
...
.cv-hint-dots span{
  display:block; width:5px; height:5px;
  border-radius:50%; background:rgba(200,218,255,.5);
}
.cv-hint-dots span:nth-child(2){ opacity:.5; }
.cv-hint-dots span:nth-child(3){ opacity:.22; }
@keyframes hint-fall{
  0%,100%{ transform:translateY(0); }
  50%    { transform:translateY(6px); }
}

/* INVITE — THE SCROLL CONTAINER */
.invite{
  position:fixed; top:0; left:0; right:0; bottom:0;
  z-index:100;
  overflow-x:hidden;
...
.floral-bg{
  position:fixed; top:0; left:0; right:0; bottom:0;
  background-image:var(--floral-url);
  background-size:cover; background-position:center;
...
.hero::before{ top:0; background:linear-gradient(to bottom,rgba(247,245,242,.95),transparent); }
.hero::after { bottom:0; background:linear-gradient(to top,rgba(247,245,242,.95),transparent); }
.hero-vid{
  position:absolute; top:0; left:0; right:0; bottom:0;
...
.hero-floral{
  position:absolute; top:0; left:0; right:0; bottom:0;
  background-image:var(--floral-url);
  background-size:cover; opacity:.04; mix-blend-mode:screen;
}
.hero-overlay{
  position:absolute; top:0; left:0; right:0; bottom:0;
  background:linear-gradient(180deg,rgba(247,245,242,.6) 0%,rgba(247,245,242,.1) 35%,rgba(247,245,242,.1) 65%,rgba(247,245,242,.88) 100%);
}
.hero-body{
  position:relative; z-index:2;
  padding:28px 40px;
  background:rgba(247,245,242,.72); border-radius:8px;
  opacity:0; transform:translateY(28px);
  animation:hero-in 1.6s cubic-bezier(.22,1,.36,1) .35s forwards;
}
@keyframes hero-in{ to{ opacity:1; transform:translateY(0); } }
.hero-tag{
  font-family:'Cinzel',serif; font-size:clamp(9px,2vw,11px);
  letter-spacing:.55em; text-transform:uppercase;
  color:rgba(30,52,112,.7); margin-bottom:22px;
}
.hero-names{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(56px,14vw,100px);
  font-weight:300; line-height:.88; color:#0e1e5a;
  text-shadow:0 1px 4px rgba(247,245,242,.8);
}
.hero-names em{
  display:block; font-style:italic; color:#1e3470;
  text-shadow:0 1px 4px rgba(247,245,242,.8);
}
.hero-amp{
  display:block; font-family:'Cormorant Garamond',serif;
  font-size:.44em; font-style:italic; color:rgba(58,90,155,.35); margin:.16em 0;
}
.hero-date{
  font-family:'Cinzel',serif; font-size:clamp(10px,2.4vw,13px);
  letter-spacing:.4em; color:rgba(30,52,112,.72); margin-top:10px;
}
.hero-line{
  width:1px; height:50px;
  background:linear-gradient(to bottom,transparent,rgba(58,90,155,.35),transparent);
  margin:22px auto;
}
.hero-scroll{
  font-family:'Cormorant Garamond',serif; font-style:italic;
  font-size:clamp(13px,3vw,14px); color:rgba(58,90,155,.45); letter-spacing:.1em;
  animation:breathe 3s ease-in-out infinite;
}
@keyframes breathe{ 0%,100%{ opacity:.4; } 50%{ opacity:.85; } }

/* SHARED */
.sec{ padding:80px 28px; max-width:520px; margin:0 auto; text-align:center; }
.sec-tag{
  font-family:'Cinzel',serif; font-size:clamp(8px,2vw,9px);
  letter-spacing:.65em; text-transform:uppercase;
  color:rgba(58,90,155,.75); margin-bottom:20px;
}
.sec-title{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(30px,7.5vw,46px); font-weight:400;
  color:#102060; line-height:1.1; margin-bottom:16px;
}
.sec-title em{ font-style:italic; color:var(--blue); }
.sec-body{
  font-family:'Cormorant Garamond',serif;
  font-size:clamp(17px,4.2vw,20px); line-height:1.85;
  font-weight:300; color:rgba(18,34,70,.88);
}
.rule{
  width:48px; height:1px;
  background:linear-gradient(90deg,transparent,var(--line),transparent);
...
.ornament{
  display:block; font-family:'Cormorant Garamond',serif;
  font-size:16px; letter-spacing:.35em;
  color:rgba(58,90,155,.35); margin:0 auto 20px;
}
.frost{
  background:rgba(250,249,247,.82);
  backdrop-filter:blur(12px); -webkit-backdrop-filter:blur(12px);
  border-radius:12px; padding:52px 28px; margin:0 -4px;
}

/* COUNTDOWN */
.cd-wrap{ position:relative; overflow:hidden; }
.cd-img{
  position:absolute; top:0; left:0; right:0; bottom:0;
  background-image:var(--table-url);
  background-size:cover; background-position:center;
  transform:scale(1.06); filter:brightness(.22) saturate(.62);
}
.cd-inner{ position:relative; z-index:2; padding:80px 28px; max-width:520px; margin:0 auto; text-align:center; }
.cd-grid{ display:grid; grid-template-columns:repeat(4,1fr); gap:10px; margin-top:36px; }
.cd-unit{
  background:rgba(255,255,255,.88); border:1px solid var(--line);
  border-radius:5px; padding:20px 8px 15px; position:relative;
  backdrop-filter:blur(8px);
}
.cd-unit::before{
...
.cd-num{
  ...
  font-size:clamp(28px,7vw,44px); font-weight:500; color:#102060;
  line-height:1; margin-bottom:8px;
}
.cd-lbl{
  font-family:'Cormorant Garamond',serif;
  font-size:clamp(9px,2.2vw,11px); letter-spacing:.22em; text-transform:uppercase;
  color:var(--muted);
}

/* SCRATCH */
.scratch-sec{ padding:80px 28px; text-align:center; }
.scratch-frame{
  position:relative; width:min(290px,78vw); height:234px;
  margin:32px auto 0; border-radius:6px; overflow:hidden; cursor:crosshair;
  box-shadow:0 0 0 1px var(--line),0 8px 40px rgba(26,42,74,.12);
}
.scratch-reveal{
  position:absolute; top:0; left:0; right:0; bottom:0;
  width:100%; height:100%; object-fit:contain;
  object-position:center; background:#dce8f5; display:block;
}
.scratch-canvas{
  position:absolute; top:0; left:0; right:0; bottom:0;
  width:100%; height:100%; touch-action:none;
...
.scratch-hint{
  font-family:'Cormorant Garamond',serif; font-style:italic;
  font-size:clamp(13px,3.2vw,15px); color:var(--muted);
  letter-spacing:.08em; margin-top:18px;
}
.scratch-msg{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(18px,4.5vw,23px); font-weight:300; color:var(--blue);
  margin-top:22px; opacity:0; transition:opacity 1.3s ease;
}
.scratch-msg.on{ opacity:1; }

/* DRESS CODE */
.dress-sec{ padding:80px 28px 72px; text-align:center; }
.dress-card{
  max-width:440px; margin:32px auto 0;
...
.dress-img-wrap{
  width:100%; background:rgba(248,250,255,.5);
  padding:24px 20px 10px;
  display:flex; align-items:center; justify-content:center;
}
.dress-img{ width:100%; max-width:380px; height:auto; display:block; object-fit:contain; }
.dress-body{ padding:20px 26px 30px; }
.dress-title{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(20px,5vw,26px); font-weight:400; color:var(--blue-d);
  letter-spacing:.06em; margin-bottom:10px;
}
.dress-divider{
  width:36px; height:1px;
...
.dress-note{
  font-family:'Cormorant Garamond',serif;
  font-size:clamp(16px,4vw,18px); line-height:1.75;
  font-weight:300; font-style:italic; color:rgba(26,42,74,.7);
}
.dress-tag{
  display:inline-block; margin-top:14px;
...
.rsvp-vid-overlay{
  position:absolute; top:0; left:0; right:0; bottom:0;
  background:rgba(247,245,242,.45); z-index:1; pointer-events:none;
}
.rsvp-inner{ position:relative; z-index:2; padding:80px 28px 90px; max-width:500px; margin:0 auto; }
.rsvp-form{ display:flex; flex-direction:column; gap:15px; margin-top:34px; text-align:left; }
.f-lbl{
  display:block; font-family:'Cinzel',serif;
  font-size:9px; letter-spacing:.4em; text-transform:uppercase;
  color:var(--muted); margin-bottom:6px;
...
.f-input,.f-textarea{
  width:100%; background:rgba(255,255,255,.92);
  border:1px solid var(--line); border-radius:3px;
  padding:13px 15px; font-family:'Cormorant Garamond',serif;
  font-size:17px; color:var(--text); outline:none; font-weight:300;
  transition:border-color .3s; -webkit-appearance:none;
}
.f-input::placeholder,.f-textarea::placeholder{ color:rgba(58,90,155,.3); }
.f-input:focus,.f-textarea:focus{ border-color:rgba(58,90,155,.45); }
.f-textarea{ resize:none; min-height:78px; }
.f-choices{ display:grid; grid-template-columns:1fr 1fr; gap:10px; }
.f-choice{
  padding:13px 10px; border:1px solid var(--line); border-radius:3px;
  background:rgba(255,255,255,.6); font-family:'Cormorant Garamond',serif;
  font-size:16px; color:rgba(58,90,155,.62);
  cursor:pointer; text-align:center; transition:all .3s;
  -webkit-tap-highlight-color:transparent;
}
.f-choice.sel{
  border-color:rgba(58,90,155,.6); background:rgba(58,90,155,.08); color:var(--blue-d);
}
.guests-list{ display:flex; flex-direction:column; gap:8px; }
.g-row{ display:flex; gap:8px; align-items:center; }
.g-row .f-input{ flex:1; }
...
.g-btn:hover{ background:rgba(58,90,155,.06); color:var(--blue-d); }
.g-add{
  width:100%; padding:12px; border:1px dashed var(--line); border-radius:3px;
  background:transparent; font-family:'Cormorant Garamond',serif;
  font-size:15px; font-style:italic; color:rgba(58,90,155,.4);
  cursor:pointer; text-align:center; transition:all .25s;
  -webkit-tap-highlight-color:transparent;
}
.g-add:hover{ border-color:rgba(58,90,155,.38); color:var(--blue); }
.f-submit{
  margin-top:8px; padding:16px 32px; background:transparent;
  border:1px solid rgba(58,90,155,.4); border-radius:3px;
  font-family:'Cinzel',serif; font-size:10px; letter-spacing:.45em; text-transform:uppercase;
  color:var(--blue); cursor:pointer; position:relative; overflow:hidden;
  transition:border-color .3s; -webkit-tap-highlight-color:transparent;
}
.f-submit::after{
  content:''; position:absolute; inset:0; background:rgba(58,90,155,.06);
...
/* VENUE */
.venue-sec{ padding:80px 28px; text-align:center; }
.venue-card{
  max-width:430px; margin:0 auto; border:1px solid var(--line);
  border-radius:6px; overflow:hidden; background:rgba(255,255,255,.9);
  backdrop-filter:blur(8px); box-shadow:0 4px 30px rgba(26,42,74,.08);
}
.venue-img{
  width:100%; height:220px; object-fit:cover; object-position:center top;
  display:block; filter:brightness(.82) saturate(.75);
}
.venue-body{ padding:26px 22px 30px; }
.venue-name{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(20px,5vw,26px); font-weight:400; color:var(--blue-d); margin-bottom:8px;
}
.venue-detail{
  font-family:'Cormorant Garamond',serif;
  font-size:clamp(14px,3.5vw,17px); color:rgba(26,42,74,.65); line-height:1.65; font-weight:300;
}
.map-btns{ display:grid; grid-template-columns:1fr 1fr; gap:9px; margin-top:22px; }
.map-btn{
  display:flex; flex-direction:column; align-items:center; justify-content:center;
  gap:7px; padding:13px 8px; border:1px solid var(--line); border-radius:4px;
  background:rgba(255,255,255,.5); text-decoration:none; cursor:pointer;
  transition:background .3s,border-color .3s; -webkit-tap-highlight-color:transparent;
}
.map-btn:hover{ background:rgba(58,90,155,.06); border-color:rgba(58,90,155,.38); }
.map-btn svg{ width:20px; height:20px; }
.map-btn span{
  font-family:'Cinzel',serif; font-size:8px; letter-spacing:.25em; text-transform:uppercase;
  color:var(--muted);
}

/* FOOTER */
.footer{ padding:52px 28px 68px; text-align:center; border-top:1px solid var(--line); }
.footer-name{
  font-family:'Cormorant Upright',serif;
  font-size:clamp(16px,4vw,20px); font-weight:300;
  font-style:italic; color:rgba(58,90,155,.65); letter-spacing:.06em;
}

/* MUSIC BUTTON */
.music-btn{
  position:fixed; bottom:30px; right:20px; z-index:500;
  display:flex; align-items:center; gap:8px;
  padding:9px 16px 9px 13px; border-radius:40px;
  border:1px solid rgba(58,90,155,.28);
  background:rgba(247,245,242,.92);
  backdrop-filter:blur(12px); -webkit-backdrop-filter:blur(12px);
  cursor:pointer; box-shadow:0 2px 20px rgba(26,42,74,.1);
  transition:all .35s cubic-bezier(.22,1,.36,1);
  -webkit-tap-highlight-color:transparent;
}
.music-btn:hover{
  background:rgba(247,245,242,1); border-color:rgba(58,90,155,.5);
  box-shadow:0 4px 28px rgba(26,42,74,.15); transform:translateY(-2px);
}
.music-btn-icon{ width:18px; height:18px; flex-shrink:0; }
.music-btn-label{
...
.mbar{ animation:mbar .75s ease-in-out infinite alternate; transform-origin:bottom; }
.mbar:nth-child(2){ animation-delay:.15s; }
.mbar:nth-child(3){ animation-delay:.3s; }
@keyframes mbar{ from{ transform:scaleY(.25); } to{ transform:scaleY(1); } }

/* TOAST */
.toast{
  position:fixed; bottom:36px; left:50%;
  transform:translateX(-50%) translateY(14px);
  background:rgba(247,245,242,.96); border:1px solid var(--line);
  border-radius:4px; padding:13px 28px;
  font-family:'Cormorant Garamond',serif; font-size:16px; color:var(--blue);
